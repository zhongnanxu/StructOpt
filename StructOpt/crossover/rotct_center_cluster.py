...
def rotct_center_cluster(ind1, ind2, Optimizer):
    ...
    if 'CX' in Optimizer.debug:
        ...
        debug = False
    Optimizer.output.write('Rotate Cut/Splice Cx between individual {} and individual {}\n'.format(repr(ind1.index), repr(ind2.index)))
    ...
    # We try this at most 10 times
    n = 0
    while n < 10:
        # Rotate a random degrees around a random axis
        rax = random.choice(['x', '-x', 'y', '-y', 'z', '-z'])
        rang = random.random()*90
        indi1.rotate(rax, a=rang, rotate_cell=False)
        ...
        # Assure at least two atoms are cut. If not, try again
    ...
    indi2.rotate(rax, a=rang, rotate_cell=False)
    ...
    # If atoms were deleted, add the atoms back on
    ...
    indi1.rotate(rax, a=-1*rang, rotate_cell=False)
    indi2.rotate(rax, a=-1*rang, rotate_cell=False)
