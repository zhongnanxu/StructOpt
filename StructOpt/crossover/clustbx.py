...
def clustbx(ind1, ind2, Optimizer):
    ...
    if 'CX' in Optimizer.debug:
        ...
        debug = False
    Optimizer.output.write('Box Cluster Cx between individual {} and individual {}\n'.format(ind1.index, repr(ind2.index)))
    ...
    for sym, c, m, u in Optimizer.atomlist:
            ...
            Optimizer.output.write('CX(clustbx):New solid1 contains {} {} atoms\n'.format(repr(nc), repr(sym)))
            nc = len([atm for atm in nsolid2 if atm.symbol == sym])
            Optimizer.output.write('CX(clustbx):New solid2 contains {} and {} atoms\n'.format(repr(nc), repr(sym)))
    if Optimizer.forcing != 'Concentration':
