...
import logging
import json
from StructOpt.structoptio.logger_utils import initialize_logger
from StructOpt.tools.check_atomlist_concentration import check_atomlist_concentration
import pdb

def read_parameter_input(input):
    ...
    parameters.setdefault('USE_MPI4PY', True)
    parameters.setdefault('filename', 'Output')
...
def set_default_parameters(parameters):
    ...
    parameters.setdefault('parallel', False)

    if 'modules' not in parameters:
        logger.warning('modules not set.  Default values set to LAMMPS.\n')
    parameters.setdefault('modules', ['LAMMPS'])

    if 'LAMMPS' in parameters['modules'] and 'VASP' in parameters['modules']:
        raise ValueError("The input variable 'modules' cannot contain both VASP and LAMMPS")

    if 'LAMMPS' in parameters['modules']:
        parameters.setdefault('relaxations', ['LAMMPS'])
    elif 'VASP' in parameters['modules']:
        parameters.setdefault('relaxations', ['VASP'])
    else:
        parameters.setdefault('relaxations', [])
