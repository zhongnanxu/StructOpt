...
def write_optimizer(Optimizer, optfile, restart=True):
    ...
    stemlist = [ 'psf_file',
        ...
        'source_size',
        'slice_size',
        'chromatic_aberration_coefficient',
        'delta_E',
        'aber',
        'scale_factor',
        'grid_sim2exp',
    ...
    #Write Optimizer STEM parameters
    for one in stemlist:
        try:
            optpar = eval('Optimizer.{0}'.format(one))
            if one == 'psf_file' or one == 'stem_ref':
                optfile.write("'{0}':'{1}',\n".format(one,optpar.split('/')[-1]))
            elif isinstance(optpar,str):
                if '\n' in optpar:
                    optfile.write("'{0}':{1},\n".format(one,repr(optpar)))
                else:
                    optfile.write("'{0}':'{1}',\n".format(one,optpar))
            ...
            optfile.write("'{0}':{1},\n".format(one,None))
            #print 'Cannot write STEM parameters: {0}'.format(one) 
